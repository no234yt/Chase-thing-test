...
local CONFIG = {
	...
	FLICKER_START_TRANSPARENCY = 0.5,
	FLICKER_END_TRANSPARENCY = 1.0,
	FLICKER_FADE_TIME = 0.2,
	FLICKER_BASE_WAIT = 0.4,
	FLICKER_MIN_WAIT = 0.1,
...
local function requestJump()
	if not State.canJump then return end
	State.canJump = false
...
local function ensureJumpHeight()
	task.spawn(function()
		while true do
			task.wait(0.1)
			local char = player.Character
...
local function createHighlight()
	...
	State.highlight.FillTransparency = CONFIG.FLICKER_START_TRANSPARENCY
...
local function flickerHighlight()
	...
	State.flickerCoroutine = task.spawn(function()
		while State.isCharging and State.highlight and State.flickerSound do
			local waitTime = lerp(CONFIG.FLICKER_BASE_WAIT, CONFIG.FLICKER_MIN_WAIT, State.chargePercent)
			...
			while tick() - fadeStartTime < CONFIG.FLICKER_FADE_TIME and State.isCharging do
				...
				State.highlight.FillTransparency = lerp(
					CONFIG.FLICKER_START_TRANSPARENCY,
					CONFIG.FLICKER_END_TRANSPARENCY,
					fadeProgress
				)
				task.wait()
			end
			...
			if State.highlight then
				State.highlight.FillTransparency = CONFIG.FLICKER_END_TRANSPARENCY
			end
